...
const PublicStateTable = () => {
    ...
    const fetchStates = async () => {
        const response = await axios.get('http://localhost:8080/privateeventcontroller/publicstates');
    ...
    const handleAddState = async () => {
        if (newStateName) {
            const response = await axios.post('http://localhost:8080/privateeventcontroller/publicstates', { stateName: newStateName });
    ...
    const handleEditState = async (state) => {
        ...
        if (updatedStateName) {
            ...
            await axios.put(`http://localhost:8080/privateeventcontroller/publicstates/${state.stateId}`, updatedState);
            fetchStates();
    ...
    const handleDeleteState = async (stateId) => {
        await axios.delete(`http://localhost:8080/privateeventcontroller/publicstates/${stateId}`);
        fetchStates();
    };
    ...
    return (
        <TableContainer component={Paper} className='table-container'>
            <Table>
                <TableHead>
                    <TableRow>
                        <TableCell className='cell-head-font'>State Name</TableCell>
                    ...
                    {states.map((state) => (
                        <TableRow key={state.stateId}>
                            ...
                            <TableCell>
                                ...
                                <Button component={Link} to={`/publiccities/${state.stateId}`} className="button-styles">View Cities</Button>
                            </TableCell>
                        </TableRow>
                    ))}
                    <TableRow>
...
export default PublicStateTable;
